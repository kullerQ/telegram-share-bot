...
- TikTok **photo posts** (image slideshows with sound) are compiled into an MP4 via `ffmpeg`: each image is shown for about `SLIDESHOW_SLIDE_MS` (default 2500 ms), the video length matches the **full** audio track, and images loop if needed to fill that length. If there are more images than the preferred cadence allows, slide time is shortened so every image still appears. At most `SLIDESHOW_MAX_IMAGES` (default 35) images are included. The Docker image already ships a static `ffmpeg`.
