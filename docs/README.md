...
Inline mode offers explicit Video and Audio results; YouTube clip links also offer clip and full-length versions in both formats. In a private chat, paste a link to use your saved format default, or choose from the menu when it is set to **Not specified**. Use `/settings` to set quality (Auto, Best, or Balanced), caption (Custom or Media title), and format (Not specified, Video, or Audio). **Custom** uses text you add after the link or clip range; without that text, it sends no caption. **Media title** shows the title as a link to the original video. An explicit caption overrides a saved caption choice. The bottom **Reset to default** button restores Auto quality, the bot's configured caption behavior, and Not specified format. Auto starts with the best source and switches quality when the estimated download exceeds 45 seconds by default. Best never steps down to another source format and reports when that source cannot fit Telegram. Balanced favors a good picture with a smaller download. Quality preferences are not literal height caps. `/video`, `/video best`, `/video balanced`, and `/audio` provide explicit per-request choices.
