...
For a YouTube **clip**, put a time range as the first token after the link (`start-end`, e.g. `1:20-2:05` or `90-150`). Inline mode offers **Send clip** and **Send full video**; in a private chat the bot asks with buttons. Captions may follow the range. A caption without a leading range still downloads the whole video.
...
- YouTube clips: optional `start-end` after the link (max 10 minutes). Requires `ffmpeg` (already in the Docker image).
