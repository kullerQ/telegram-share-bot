...
from sendmedia_bot.config import DEFAULT_MAX_CONCURRENT_DOWNLOADS, load_settings
from sendmedia_bot.downloader import DownloadedMedia, MediaKind
from sendmedia_bot.handlers import _download_semaphore, url_message
...
class TestConcurrencyLimiter(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_semaphore_limits_parallel_downloads(self) -> None:
        ...
        with (
            patch("sendmedia_bot.handlers.get_direct_stream", AsyncMock(return_value=None)),
            patch("sendmedia_bot.handlers.download_media", side_effect=fake_download),
            patch("sendmedia_bot.handlers._send_media_to_chat", AsyncMock()),
            patch(
                "sendmedia_bot.handlers._file_id_and_kind_from_message",
                return_value=("file_123", MediaKind.VIDEO),
            ),
            patch("sendmedia_bot.handlers.cleanup_media"),
        ):
